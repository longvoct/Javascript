const singleNode = document.querySelector("h1");
console.log(singleNode);
const singleNode2 = document.querySelector(".container");
console.log(singleNode2);
const singleNode3 = document.querySelector("#spinner");
console.log(singleNode3);

const nodeItem = document.querySelector(".item");
console.log(nodeItem);

const nodeList = document.querySelectorAll(".item");
console.log(nodeList);

const nodeList2 = document.querySelectorAll(".item");
console.log(nodeList);

[...nodeList2].map((item) => item);
// console.log(nodeList2);
for (let i = 0; i < nodeList2.length; i++) {
  console.log(nodeList2[i]);
}

const classNodes = document.getElementsByClassName("item"); // ít người sử dụng
console.log(classNodes);

const tagNodes = document.getElementsByTagName("li");
console.log(tagNodes);

[...tagNodes].forEach((item) => {
  console.log(item);
});
